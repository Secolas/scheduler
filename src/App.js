...
import { Plus, X, Calendar, GripVertical, Edit2, Check, Trash2, LayoutGrid, Columns, RotateCcw, Eraser, CalendarDays } from 'lucide-react';
...
export default function WeeklyScheduler() {
  ...
  }, [viewMode]); // Added viewMode dependency to re-trigger scroll on view change
  ...
  const handleAddItem = (day) => {
    ...
    setSchedule(prev => {
      ...
      updatedPlans[dayData.activePlanIndex] = {
        ...activePlan,
        items: [...activePlan.items, { id: Date.now(), text: val, completed: false }]
      };
  ...
  // MOBILE FRIENDLY FIX: Force text-base on inputs to prevent iOS zoom
          ...
          <div className="flex flex-wrap gap-2 sm:ml-auto">
            ...
            <div className="flex bg-gray-200 p-1 rounded-lg">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${viewMode === 'landscape' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${viewMode === 'grid' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                  ...
                  {dayData.plans.map((plan, idx) => {
                    ...
                    return (
                      <div 
                        ...
                        onDoubleClick={() => startEditingPlan(day, idx, plan.name)}
                      >
                        {isEditingThisPlan ? (
                          <input
                            autoFocus
                            type="text"
                            className="w-12 bg-transparent outline-none border-b border-indigo-500 text-base sm:text-sm"
                            value={editPlanValue}
                        ...
                        {dayData.plans.length > 1 && (
                          <button
                            ...
                            className={`ml-1 hover:text-red-500 ${isActive ? 'block' : 'hidden group-hover:block'}`}
                          >
                  ...
                  {items.map((item, index) => {
                    ...
                    return (
                      <li 
                        ...
                        onDragStart={(e) => handleDragStart(e, item, day, dayData.activePlanIndex, index)}
                        onDragEnter={(e) => handleDragEnter(e, day, dayData.activePlanIndex, index)}
                        ...
                        className={`
                          ...
                          ${dragItem.current?.item.id === item.id ? 'opacity-50 bg-indigo-100 scale-95' : 'hover:bg-white hover:scale-[1.02]'}
                      ...
                      >
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleCompletion(day, dayData.activePlanIndex, item.id);
                          }}
                          className={`
                          ...
                          {isEditingThis ? (
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                className={`w-full bg-transparent border-b border-indigo-300 focus:border-indigo-500 outline-none px-1 text-base sm:text-sm`}
                              />
                              <button onClick={saveEdit} className="text-green-600 hover:bg-green-50 rounded p-0.5">
                                <Check className="w-3 h-3" />
                              </button>
                            </div>
                          ) : (
                            <span 
                              className={`${textClasses} font-medium leading-snug break-words block cursor-text transition-all duration-200 ${item.completed ? 'line-through text-gray-400' : 'text-gray-800'}`}
                              onDoubleClick={() => startEditing(day, dayData.activePlanIndex, item)}
                            >
                              {item.text}
                            </span>
                          )}
                        ...
                        {!isEditingThis && (
                          <div className="flex opacity-0 group-hover:opacity-100 transition-opacity duration-200 gap-1">
                             <button
                              onClick={() => startEditing(day, dayData.activePlanIndex, item)}
              ...
              <div className={`border-t border-black/5 bg-white/30 rounded-b-xl ${viewMode === 'landscape' ? 'p-2' : 'p-3 mt-auto'}`}>
                <div className="flex gap-1">
                  <input
                    type="text"
                    value={newItems[day] || ""}
                    onChange={(e) => setNewItems(prev => ({ ...prev, [day]: e.target.value }))}
                    onKeyDown={(e) => e.key === 'Enter' && handleAddItem(day)}
                    placeholder="Add..."
                    className={`flex-grow px-2 py-1 rounded border border-transparent focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 bg-white shadow-sm outline-none transition-all placeholder-gray-400 text-base sm:text-sm`}
                  />
                  <button
